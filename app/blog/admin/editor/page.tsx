...
function BlogEditorInner() {
  ...
  const [author, setAuthor] = useState("Mohawk Medicine Team");
  ...
  useEffect(() => {
    ...
    fetch(`${APPS_SCRIPT_URL}?action=blog&store=MCM01&admin=1`)
      .then((r) => r.json())
      .then((data) => {
        ...
        if (post) {
          ...
          setAuthor(post.author || "Mohawk Medicine Team");
  ...
  async function handleSave() {
    ...
    const payload = editId
      ...
      : { action: "create", title, content, author, published, store: "MCM01" };
