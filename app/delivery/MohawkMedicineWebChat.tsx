...
async function preparePhoto(file: File) {
  ...
  try {
    ...
    try {
      await new Promise<void>((resolve, reject) => { image.onload = () => resolve(); image.onerror = () => reject(new Error("We could not open that photo. Please retake it.")); image.src = objectUrl; });
    } catch (error) { URL.revokeObjectURL(objectUrl); throw error; }
    source = image; width = image.naturalWidth; height = image.naturalHeight; cleanup = () => URL.revokeObjectURL(objectUrl);
  }
  if (!width || !height) { cleanup(); throw new Error("We could not open that photo. Please retake it."); }
  const scale = Math.min(1, 1600 / Math.max(width, height));
  const canvas = document.createElement("canvas");
  canvas.width = Math.max(1, Math.round(width * scale)); canvas.height = Math.max(1, Math.round(height * scale));
  const context = canvas.getContext("2d");
  if (!context) { cleanup(); throw new Error("We could not prepare that photo on this phone."); }
  try { context.drawImage(source, 0, 0, canvas.width, canvas.height); } finally { cleanup(); }
  let quality = 0.86;
...
export default function MohawkMedicineWebChat() {
  ...
  const refreshAvailability = useCallback(async () => {
    ...
    catch { setAvailability(null); setAvailabilityUnavailable(true); }
  }, []);

  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous Mohawk Medicine chat could not be restored."));
      void fetch(`${API_BASE}/api/web-chat/id-review`, { cache: "no-store" }).then(payload).then((data) => setUploadAvailable(Boolean(data.upload?.available))).catch(() => setUploadAvailable(false));
  ...
  async function start(event: FormEvent) {
    ...
    if (!intent) { setNotice("Choose whether you are new or returning."); return; }
    if (!availability || availability.state !== "AVAILABLE") { setNotice(availability?.message || "Delivery status is temporarily unavailable. Please check back soon."); return; }
    setBusy(true); setNotice("");
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/session`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ storeId: "MCM01", customerName: intent === "NEW_CUSTOMER" ? name : "", phone, intent, message: firstMessage, workflowVersion: "READY_V1", smsConsent }) }));
  ...
  function selectPhoto(file?: File) {
    if (!file) return; if (previewRef.current) URL.revokeObjectURL(previewRef.current);
  ...
  return <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="Mohawk Medicine Web Chat">
    ...
    {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="Mohawk Medicine Web Chat">
      ...
      {!token ? (!availability || availability.state !== "AVAILABLE" ? <div className="sod-chat-start sod-chat-paused"><p>{statusMessage || "Checking delivery availability..."}</p><button type="button" onClick={() => void refreshAvailability()}>Check again</button></div> : <form className="sod-chat-start" onSubmit={start}>
        ...
        <div className="sod-chat-transcript" aria-live="polite">{(conversation?.messages || []).map((item) => <div className={item.direction} key={item.id}><span>{item.body}</span><small>{new Date(item.at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</small></div>)}</div>
        {reviewStatus === "PENDING_REVIEW" && <p className="sod-id-pending" role="status">ID sent - waiting for dispatcher review.</p>}
        {retryNeeded && !retryExpanded && <div className="sod-id-retry-prompt" role="status"><span>{reviewStatus === "REJECTED" ? "ID photo was not approved." : "ID photo expired."}</span><button type="button" onClick={() => setRetryExpanded(true)}>Try ID photo again</button></div>}
        {showUpload && <section className="sod-id-upload"><strong>New customer ID verification</strong><p>Take one clear selfie while holding your government-issued photo ID beside your face.</p><input ref={fileInput} hidden type="file" accept="image/jpeg,image/png,image/webp" capture="user" onChange={(event) => selectPhoto(event.target.files?.[0])} /><button type="button" disabled={busy || !uploadAvailable} onClick={() => fileInput.current?.click()}>{!uploadAvailable ? "Private camera upload unavailable" : busy ? "Sending private photo..." : photoPreview ? "Retake selfie with ID" : "Take selfie with ID"}</button>
          ...
          {uploadState !== "idle" && <p className={`sod-id-upload-status ${uploadState === "error" ? "error" : ""}`} role="status">{uploadState === "preparing" ? "Preparing your private photo..." : uploadState === "uploading" ? "Sending privately to the dispatcher..." : uploadState === "sent" ? "Photo sent privately for dispatcher review." : uploadError}</p>}
